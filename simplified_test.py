...
def generate_test_images():
    ...
    try:
        # Test Case 1: GOOD - Single character, simple scene with enhanced negative prompts
        ...
        good_negative = (
            ...
            "many colors, colorful, rainbow, gradients, patterns, textures, "
            "detailed, busy, complicated"
        )
        ...
        if good_image:
            ...
            test_cases.append({
                ...
                "path": good_path,
                "prompt": good_prompt,
                "simple_prompt": "boy brushing teeth bathroom",  # Simple for BLIP
        ...
        if bad_image:
            ...
            test_cases.append({
                ...
                "path": bad_path,
                "prompt": bad_prompt,
                "simple_prompt": "children classroom teacher",  # Simple for BLIP
        ...
        medium_prompt = (
            ...
            "kitchen items visible, kitchen appliances, food on table, "
            "one character, detailed kitchen scene, colorful breakfast"
        )
        medium_negative = (
            "multiple people, realistic photo, "
            "simple background, plain background, minimal, empty, "
            "few colors, monochrome, minimalist"
        )
        ...
        if medium_image:
            ...
            test_cases.append({
                ...
                "simple_prompt": "boy eating breakfast table",  # Simple for BLIP
...
def test_evaluation_framework(test_cases):
    ...
    try:
        ...
        # Evaluate each test case with simple prompts
        for i, test_case in enumerate(test_cases, 1):
            print(f"\n📊 EVALUATING {i}/{len(test_cases)}: {test_case['name']}")
            print("-" * 50)
            print(f"Expected: {test_case['expected']}")
            print(f"Using simple prompt: '{test_case['simple_prompt']}'")
            
            # Run evaluation with simple prompt for better BLIP matching
            result = evaluator.evaluate_single_image(
                image=test_case['path'],
                prompt=test_case['simple_prompt'],  # Use simple prompt
                save_report=True,
            ...
            if 'category_scores' in result:
                print(f"   Categories:")
                print(f"     - Simplicity: {result['category_scores']['simplicity']:.3f}")
                print(f"     - Accuracy: {result['category_scores']['accuracy']:.3f}")
